sap.ui.define([
    "com/sap490/defectmgmt/controller/BaseController",
    "sap/ui/model/Filter",
    "sap/ui/model/FilterOperator",
    "sap/ui/model/Sorter",
    "sap/m/MessageToast",
    "sap/m/MessageBox"
], function (BaseController, Filter, FilterOperator, Sorter, MessageToast, MessageBox) {
    "use strict";

    return BaseController.extend("com.sap490.defectmgmt.controller.CreateIssue", {

        /**
         * Lifecycle hook — called when view is initialized.
         */
        onInit: function () {
            // Register router target matched
            this.getRouter().getRoute("CreateIssue").attachPatternMatched(this._onRouteMatched, this);
        },

        /**
         * Route match event handler. Resets form fields.
         */
        _onRouteMatched: function () {
            this._resetForm();
        },

        /**
         * Resets form values and validation states.
         */
        _resetForm: function () {
            this.byId("inpTitle").setValue("").setValueState("None");
            this.byId("txtDescription").setValue("").setValueState("None");
            
            var oSelModule = this.byId("selModule");
            oSelModule.setSelectedKey("");
            oSelModule.setValueState("None");
            
            this.byId("selSeverity").setSelectedKey("LOW");
            this.byId("inpAffectedVersion").setValue("1.0");
            
            var oDpDueDate = this.byId("dpDueDate");
            oDpDueDate.setValue("");
            oDpDueDate.setValueState("None");
            
            var oDeveloperSelect = this.byId("selDeveloper");
            oDeveloperSelect.setEnabled(false);
            oDeveloperSelect.destroyItems();
            oDeveloperSelect.addItem(new sap.ui.core.Item({
                key: "",
                text: "Select Module First / Auto-Assign"
            }));
            oDeveloperSelect.setSelectedKey("");
            
            this.byId("txtDeveloperWorkloadInfo").setText("");
        },

        /**
         * Triggered when any required input field changes. Resets value states.
         */
        onFieldChange: function (oEvent) {
            var oControl = oEvent.getSource();
            if (oControl.getValue() || oControl.getSelectedKey()) {
                oControl.setValueState("None");
            }
        },

        /**
         * Triggered when module is changed.
         * Dynamically queries developers for the selected module.
         */
        onModuleChange: function (oEvent) {
            var oSelect = oEvent.getSource();
            var sModule = oSelect.getSelectedKey();
            var oDeveloperSelect = this.byId("selDeveloper");
            var oWorkloadInfo = this.byId("txtDeveloperWorkloadInfo");

            oSelect.setValueState("None");

            if (!sModule) {
                oDeveloperSelect.setEnabled(false);
                oDeveloperSelect.destroyItems();
                oDeveloperSelect.addItem(new sap.ui.core.Item({
                    key: "",
                    text: "Select Module First / Auto-Assign"
                }));
                oDeveloperSelect.setSelectedKey("");
                oWorkloadInfo.setText("");
                return;
            }

            oDeveloperSelect.setEnabled(true);
            
            // Build dynamic filters for Developer list binding:
            // 1. Modulename must equal the selected module (FI/MM/SD/HCM/PP/QM)
            // 2. Developer must be active (is_active === 'X')
            var aFilters = [
                new Filter("modulename", FilterOperator.EQ, sModule),
                new Filter("is_active", FilterOperator.EQ, "X")
            ];

            // Re-bind the items aggregation of select control
            oDeveloperSelect.bindItems({
                path: "/Developer",
                filters: aFilters,
                sorters: [new Sorter("workload_score", false)], // Ascending workload score (lowest first)
                template: new sap.ui.core.Item({
                    key: "{developer_id}",
                    text: "{developer_id} (Workload: {workload_score})"
                }),
                events: {
                    dataReceived: function () {
                        // After developers are loaded, select the first developer (lowest workload) automatically
                        var aItems = oDeveloperSelect.getItems();
                        if (aItems && aItems.length > 0) {
                            oDeveloperSelect.setSelectedItem(aItems[0]);
                            oWorkloadInfo.setText("Lowest workload developer was pre-selected automatically.");
                        } else {
                            oWorkloadInfo.setText("No active developers found for module " + sModule);
                        }
                    }
                }
            });
        },

        /**
         * Validates all required form inputs.
         * @returns {boolean} True if form is valid, false otherwise.
         */
        _validateForm: function () {
            var bValid = true;

            var oInpTitle = this.byId("inpTitle");
            if (!oInpTitle.getValue().trim()) {
                oInpTitle.setValueState("Error");
                oInpTitle.setValueStateText("Title is required");
                bValid = false;
            }

            var oTxtDescription = this.byId("txtDescription");
            if (!oTxtDescription.getValue().trim()) {
                oTxtDescription.setValueState("Error");
                oTxtDescription.setValueStateText("Description is required");
                bValid = false;
            }

            var oSelModule = this.byId("selModule");
            if (!oSelModule.getSelectedKey()) {
                oSelModule.setValueState("Error");
                oSelModule.setValueStateText("Module is required");
                bValid = false;
            }

            var oDpDueDate = this.byId("dpDueDate");
            var sDateVal = oDpDueDate.getValue();
            if (!sDateVal) {
                oDpDueDate.setValueState("Error");
                oDpDueDate.setValueStateText("Due date is required");
                bValid = false;
            } else {
                var oSelectedDate = new Date(sDateVal);
                var oToday = new Date();
                oToday.setHours(0,0,0,0);
                if (oSelectedDate < oToday) {
                    oDpDueDate.setValueState("Error");
                    oDpDueDate.setValueStateText("Due date cannot be in the past");
                    bValid = false;
                }
            }

            return bValid;
        },

        /**
         * Submits the ticket form. Creates OData V4 record.
         */
        onSubmit: function () {
            if (!this._validateForm()) {
                MessageBox.error("Please fill in all required fields and correct validation errors.");
                return;
            }

            var oView = this.getView();
            oView.setBusy(true);

            var sTitle = this.byId("inpTitle").getValue().trim();
            var sDescription = this.byId("txtDescription").getValue().trim();
            var sModule = this.byId("selModule").getSelectedKey();
            var sSeverity = this.byId("selSeverity").getSelectedKey();
            var sAffectedVersion = this.byId("inpAffectedVersion").getValue().trim() || "1.0";
            var sDueDateStr = this.byId("dpDueDate").getValue();
            var sDeveloper = this.byId("selDeveloper").getSelectedKey();

            // Format date for OData V4: YYYY-MM-DDT00:00:00Z
            var oDueDate = new Date(sDueDateStr);
            var sFormattedDueDate = oDueDate.toISOString().split("T")[0] + "T00:00:00Z";

            // Prepare payload
            var oPayload = {
                title: sTitle,
                description: sDescription,
                modulename: sModule,
                severity: sSeverity,
                status: "ASSIGNED",
                assigned_to: sDeveloper || null,
                due_date: sFormattedDueDate,
                affected_version: sAffectedVersion
            };

            var oModel = this.getModel();
            
            // Create a list binding to /Issue with a dedicated deferred update group
            var oListBinding = oModel.bindList("/Issue", null, null, null, {
                $$updateGroupId: "createGroup"
            });
            
            // Create the entity
            var oContext = oListBinding.create(oPayload);
            var that = this;

            // Wait for OData V4 creation completion
            oContext.created().then(function () {
                oView.setBusy(false);
                var sNewIssueId = oContext.getProperty("issue_id");
                MessageToast.show("Ticket created and assigned successfully.");
                
                // Navigate to details page
                that.getRouter().navTo("IssueDetail", {
                    issueId: encodeURIComponent(sNewIssueId)
                });
            }, function (oError) {
                oView.setBusy(false);
                
                // Backend creates might be disabled. Inspect the error payload.
                var sMessage = oError.message || "Unknown error occurred";
                if (sMessage.indexOf("Creating operations are disabled") >= 0 || 
                    sMessage.indexOf("SADL_ENTITY_RUNTIME/011") >= 0 || 
                    sMessage.indexOf("canceled") >= 0 ||
                    sMessage.indexOf("reset") >= 0) {
                    MessageBox.warning(
                        "Backend Limitation: The SAP backend OData service has 'create' operations disabled (SADL behavior definition constraint).\n\n" +
                        "However, the frontend has successfully prepared and validated the request payload.\n\n" +
                        "Payload sent: \n" + JSON.stringify(oPayload, null, 2),
                        {
                            title: "SAP Backend Write Constraint",
                            actions: ["OK"]
                        }
                    );
                } else {
                    MessageBox.error("Failed to create ticket: " + sMessage);
                }
            });

            // Explicitly submit batch request using our dedicated deferred group
            oModel.submitBatch("createGroup").then(function () {
                if (oModel.hasPendingChanges("createGroup")) {
                    oModel.resetChanges("createGroup");
                }
            });
        },

        /**
         * Cancels defect creation and returns to issue list.
         */
        onCancel: function () {
            this.onNavBack();
        }
    });
});
